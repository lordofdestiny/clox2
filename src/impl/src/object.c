...
static void freeUpvalue(Obj* gcNode);
...
static void initGcNode(GcNode* gcNode) {
    gcNode->node = (ilist_node_t) {NULL, NULL };
    gcNode->isMarked = false;
}

static Obj* allocateObject(const size_t size, const ObjType type) {
    Obj* object = (Obj*) reallocate(NULL, 0, size);
    initGcNode(&object->gcNode);
    addToGcList(&object->gcNode);
...
static uint32_t hashString(const char* chars, const int length) {
...
static void blackenClosure(Obj* object) {
    ...
    markObject(&closure->obj);
    for (int i = 0; i < closure->upvalueCount; i++) {
        markObject(&closure->upvalues[i]->obj);
    }
}
...
static void blackenNoOp([[maybe_unused]] Obj* object) { }
