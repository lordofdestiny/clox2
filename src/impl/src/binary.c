...
static void freeValueQueue(ValueQueue* queue) {
    FREE_ARRAY(Value, queue->values, queue->bufferCapacity);
...
static void freeGenericArray(GenericArray* array) {
    reallocate(array->elements, array->elementSize * array->capacity, 0);
...
void writeBinary(const char* source_file, ObjFunction* compiled, const char* path) {
    gcVaultPush(&compiled->obj.gcNode);
...
static ObjFunction* loadFunction(FILE* file, GenericArray* patchList) {
    ...
    gcVaultPush(&function->obj.gcNode);
...
static void loadFunctions(FILE* file, GenericArray* functions, GenericArray* patchList) {
...
static void loadStrings(FILE* file, GenericArray* strings) {
    while (!feof(file) && peek_int(file) != SEG_END_STRINGS) {
        ObjString* string = read_string(file);
        gcVaultPush(&string->obj.gcNode);
...
static void patchFunctionRefs(
    ...
    for (int i = 0; i < patchList->count; i++) {
        ...
        const Value toPatch = *GENERIC_GET(Value, functions, patch->toPatch);
        const ObjFunction* toPatchFn = AS_FUNCTION(toPatch);
        const Value patchWith = *GENERIC_GET(Value, patchSource, patch->patchWith);
        toPatchFn->chunk.constants.values[patch->position] = patchWith;
    }
}

#define GEN_ARRAYS_SIZE 2

static GenericArray* gen_arrays[GEN_ARRAYS_SIZE];

void markBinaryObjects() {
    for(int i = 0; i < GEN_ARRAYS_SIZE; i++) {
        if(gen_arrays[i] == NULL) {
            continue;
        }
        for(int j = 0; j < gen_arrays[i]->count; j++) {
            markValue(* GENERIC_GET(Value, gen_arrays[i], j));
        }    
    }
}

ObjFunction* loadBinary(const char* path) {
    addMarkHook(markBinaryObjects);
    ...
    GENERIC_INIT(Value, &functions);
    
    GenericArray strings;
    GENERIC_INIT(Value, &strings);

    gen_arrays[0] = &functions;
    gen_arrays[1] = &strings;
    ...
    Value scriptVal = *GENERIC_GET(Value, &functions, 0);
    ObjFunction* script = (ObjFunction*) AS_OBJ(scriptVal);

    gcVaultPush(&script->obj.gcNode);
    freeGenericArray(&functions);
    freeGenericArray(&strings);
    freeGenericArray(&patchList);
    gcVaultPop(1);
